/****************************************************************************************************
 * FILE:    crc.c
 * BRIEF:   Cyclic Redundancy Check (CRC) Source File
 ****************************************************************************************************/

/****************************************************************************************************
 * Definitions
 ****************************************************************************************************/

/*** CRC-8 ***/
#define CRC_CRC8_POLYNOMIAL          (0x07)
#define CRC_CRC8_CDMA2000_POLYNOMIAL (0x9B)

/****************************************************************************************************
 * Includes
 ****************************************************************************************************/

#include "crc.h"
#include <stddef.h>
#include <stdint.h>

/****************************************************************************************************
 * Constants and Variables
 ****************************************************************************************************/

/*** CRC-8 ***/
/* CRC-8 */
static const uint8_t crc_crc8LookupTable[256] =
{
    
};

/* CRC-8/CDMA2000 */
static const uint8_t crc_crc8Cdma2000LookupTable[256] =
{
    
};

/****************************************************************************************************
 * Function Definitions (Public)
 ****************************************************************************************************/

/****************************************************************************************************
 * FUNCT:	  crc_crc8CalculateLookupTable
 * BRIEF:	  Calculate Full CRC-8 (Lookup Table Calculation Method)
 * RETURN:  uint8_t: CRC-8
 * ARG:		  Data: Data Buffer To Calculate CRC-8 For
 * ARG:     DataLength: Length Of Data Buffer
 ****************************************************************************************************/
uint8_t crc_crc8CalculateLookupTable(const uint8_t * const Data, const uint16_t DataLength)
{
    uint8_t crc8 = CRC_CRC8_INITIAL_CRC8;
    uint32_t i;

    /*** Error Check ***/
    if(Data == NULL)
        return CRC_CRC8_INITIAL_CRC8;

    /*** Calculate Full CRC-8 (Lookup Table Calculation Method) ***/
    for(i = 0; i < DataLength; i++)
        crc8 = crc_crc8CalculatePartialLookupTable(Data[i], crc8);

    return crc8;
}

/****************************************************************************************************
 * FUNCT:	  crc_crc8CalculateLoop
 * BRIEF:	  Calculate Full CRC-8 (Loop Calculation Method)
 * RETURN:  uint8_t: CRC-8
 * ARG:		  Data: Data Buffer To Calculate CRC-8 For
 * ARG:     DataLength: Length Of Data Buffer
 ****************************************************************************************************/
uint8_t crc_crc8CalculateLoop(const uint8_t * const Data, const uint16_t DataLength)
{
    uint8_t crc8 = CRC_CRC8_INITIAL_CRC8;
    uint32_t i;

    /*** Error Check ***/
    if(Data == NULL)
        return CRC_CRC8_INITIAL_CRC8;

    /*** Calculate Full CRC-8 (Loop Calculation Method) ***/
    for(i = 0; i < DataLength; i++)
        crc8 = crc_crc8CalculatePartialLoop(Data[i], crc8);

    return crc8;
}

/****************************************************************************************************
 * FUNCT:	  crc_crc8CalculatePartialLookupTable
 * BRIEF:	  Calculate Partial CRC-8 (Lookup Table Calculation Method)
 * RETURN:  uint8_t: Current/Final CRC-8
 * ARG:		  Data: Data To Add To CRC-8 Calcuation
 * ARG:     crc8: Current CRC-8
 * NOTE:    Set CRC-8 Value To CRC_CRC8_INITIAL_CRC8 If First Data Byte (Data[0]); Returned CRC-8
 *          Value Should Be Used On Subsequent Data Bytes (Data[1] ... Data[n])
 ****************************************************************************************************/
uint8_t crc_crc8CalculatePartialLookupTable(const uint8_t Data, uint8_t crc8)
{
    /*** Calculate Partial CRC-8 (Lookup Table Calculation Method) ***/
    crc8 = crc_crc8LookupTable[(Data ^ crc8) ^ CRC_CRC8_INITIAL_CRC8];
    return crc8;
}

/****************************************************************************************************
 * FUNCT:	  crc_crc8PartialLoop
 * BRIEF:	  Calculate Partial CRC-8 (Loop Calculation Method)
 * RETURN:  uint8_t: Current/Final CRC-8
 * ARG:		  Data: Data To Add To CRC-8 Calcuation
 * ARG:     crc8: Current CRC-8
 * NOTE:    Set CRC-8 Value To CRC_CRC8_INITIAL_CRC8 If First Data Byte (Data[0]); Returned CRC-8
 *          Value Should Be Used On Subsequent Data Bytes (Data[1] ... Data[n])
 ****************************************************************************************************/
uint8_t crc_crc8CalculatePartialLoop(const uint8_t Data, uint8_t crc8)
{
    uint8_t bit;
    
    /*** Calculate Partial CRC-8 (Loop Calculation Method) ***/
    crc8 ^= Data;
    for(bit = 0; bit < 8; bit++)
    {
        if((crc8 & 0x80) == 0x80)
            crc8 = (uint8_t)((crc8 << 1) ^ CRC_CRC8_POLYNOMIAL);
        else
            crc8 <<= 1;
    }
    
    return crc8;
}

/****************************************************************************************************
 * FUNCT:	  crc_crc8Cdma2000CalculateLookupTable
 * BRIEF:	  Calculate Full CRC-8/CDMA2000 (Lookup Table Calculation Method)
 * RETURN:  uint8_t: CRC-8/CDMA2000
 * ARG:		  Data: Data Buffer To Calculate CRC-8/CDMA2000 For
 * ARG:     DataLength: Length Of Data Buffer
 ****************************************************************************************************/
uint8_t crc_crc8Cdma2000CalculateLookupTable(const uint8_t * const Data, const uint16_t DataLength)
{
    uint8_t crc8Cdma2000 = CRC_CRC8_CDMA2000_INITIAL_CRC8_CDMA2000;
    uint32_t i;

    /*** Error Check ***/
    if(Data == NULL)
        return CRC_CRC8_CDMA2000_INITIAL_CRC8_CDMA2000;

    /*** Calculate Full CRC-8/CDMA2000 (Lookup Table Calculation Method) ***/
    for(i = 0; i < DataLength; i++)
        crc8Cdma2000 = crc_crc8Cdma2000CalculatePartialLookupTable(Data[i], crc8Cdma2000);

    return crc8Cdma2000;
}

/****************************************************************************************************
 * FUNCT:	  crc_crc8Cdma2000CalculateLoop
 * BRIEF:	  Calculate Full CRC-8/CDMA2000 (Loop Calculation Method)
 * RETURN:  uint8_t: CRC-8/CDMA2000
 * ARG:		  Data: Data Buffer To Calculate CRC-8/CDMA2000 For
 * ARG:     DataLength: Length Of Data Buffer
 ****************************************************************************************************/
uint8_t crc_crc8Cdma2000CalculateLoop(const uint8_t * const Data, const uint16_t DataLength)
{
    uint8_t crc8Cdma2000 = CRC_CRC8_CDMA2000_INITIAL_CRC8_CDMA2000;
    uint32_t i;

    /*** Error Check ***/
    if(Data == NULL)
        return CRC_CRC8_CDMA2000_INITIAL_CRC8_CDMA2000;

    /*** Calculate Full CRC-8/CDMA2000 (Loop Calculation Method) ***/
    for(i = 0; i < DataLength; i++)
        crc8Cdma2000 = crc_crc8Cdma2000CalculatePartialLoop(Data[i], crc8Cdma2000);

    return crc8Cdma2000;
}

/****************************************************************************************************
 * FUNCT:	  crc_crc8Cdma2000CalculatePartialLookupTable
 * BRIEF:	  Calculate Partial CRC-8/CDMA2000 (Lookup Table Calculation Method)
 * RETURN:  uint8_t: Current/Final CRC-8/CDMA2000
 * ARG:		  Data: Data To Add To CRC-8/CDMA2000 Calcuation
 * ARG:     crc8Cdma2000: Current CRC-8/CDMA2000
 * NOTE:    Set CRC-8/CDMA2000 Value To CRC_CRC8_CDMA2000_INITIAL_CRC8_CDMA2000 If First Data Byte
 *          (Data[0]); Returned CRC-8/CDMA2000 Value Should Be Used On Subsequent Data Bytes
 *          (Data[1] ... Data[n])
 ****************************************************************************************************/
uint8_t crc_crc8Cdma2000CalculatePartialLookupTable(const uint8_t Data, uint8_t crc8Cdma2000)
{
    /*** Calculate Partial CRC-8/CDMA2000 (Lookup Table Calculation Method) ***/
    crc8Cdma2000 = crc_crc8Cdma2000LookupTable[(Data ^ crc8Cdma2000) ^ CRC_CRC8_CDMA2000_INITIAL_CRC8_CDMA2000];
    return crc8Cdma2000;
}

/****************************************************************************************************
 * FUNCT:	  crc_crc8Cdma2000PartialLoop
 * BRIEF:	  Calculate Partial CRC-8/CDMA2000 (Loop Calculation Method)
 * RETURN:  uint8_t: Current/Final CRC-8/CDMA2000
 * ARG:		  Data: Data To Add To CRC-8/CDMA2000 Calcuation
 * ARG:     crc8Cdma2000: Current CRC-8/CDMA2000
 * NOTE:    Set CRC-8/CDMA2000 Value To CRC_CRC8_CDMA2000_INITIAL_CRC8_CDMA2000 If First Data Byte
 *          (Data[0]); Returned CRC-8/CDMA2000 Value Should Be Used On Subsequent Data Bytes 
 *          (Data[1] ... Data[n])
 ****************************************************************************************************/
uint8_t crc_crc8Cdma2000CalculatePartialLoop(const uint8_t Data, uint8_t crc8Cdma2000)
{
    uint8_t bit;
    
    /*** Calculate Partial CRC-8/CDMA2000 (Loop Calculation Method) ***/
    crc8Cdma2000 ^= Data;
    for(bit = 0; bit < 8; bit++)
    {
        if((crc8Cdma2000 & 0x80) == 0x80)
            crc8Cdma2000 = (uint8_t)((crc8Cdma2000 << 1) ^ CRC_CRC8_CDMA2000_POLYNOMIAL);
        else
            crc8Cdma2000 <<= 1;
    }
    
    return crc8Cdma2000;
}
